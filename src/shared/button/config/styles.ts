export const buttonShapeStyles = {
	full: 'rounded-full',
	md: 'rounded-md'
}
export const buttonSizeStyles = {
	text: 'flex-inline-center',
	icon: 'flex-center p-2 flex-center aspect-square leading-[0]',
	'icon-md':
		'flex-center p-2 flex-center aspect-square leading-[0] landscape:w-[38px] landscape:h-[38px] w-[30px] h-[30px]',
	sm: 'flex-center px-2 py-1 text-body-sm landscape:h-[30px] h-[25px]',
	md: 'flex-center min-w-[3.5em] px-2 py-1 text-body-sm md:text-body-md landscape:h-[35px] h-[30px]',
	'md-lg':
		'flex-center min-w-[3.5em] h-[30px] px-2 text-body-sm md:text-body-md',
	lg: 'flex-center px-[1em] pt-[0.6em] pb-[0.4em] text-body-sm md:text-body-md min-w-[5em] h-input-h'
}
export const buttonVariants = {
	point: 'point-neon',
	blue: 'bg-blue-6 text-white',
	gray: 'bg-gray-6',
	ghost: 'hover:bg-gray-3',
	danger: 'bg-red',
	outline: 'border border-gray-5',
	link: 'hover:underline hover:underline-offset-2'
}
