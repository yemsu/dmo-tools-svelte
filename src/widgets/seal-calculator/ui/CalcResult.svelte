...
<section
	class={cn(
		'h-calc-result-h w-[calc(100%-var(--side-menu-w)-var(--content-side)*4-var(--side-ad-w))] overflow-hidden',
		'fixed bottom-[calc(var(--header-h)+var(--content-side))] left-[calc(var(--side-menu-w)+var(--content-side)*2)] z-toast',
		'port:flex-col land:bottom-content-side land:rounded-full',
		'bg-deep rounded-md border-2 border-gray-10 drop-shadow-lg'
	)}
>
	<h2 class="ir">
		<TextByLang
