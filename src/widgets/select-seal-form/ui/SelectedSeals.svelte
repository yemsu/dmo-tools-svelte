<script lang="ts">
	...
	$: mySealsByStatType = objectBy($mySeals, (mySeal) => mySeal.seal.statType)
	...
	$: statCalc = (statType: StatType) => {
		...
		sealsByStatType.forEach(({ seal, count }) => {
			...
			resultValue += seal.maxIncrease * (sealPercent / 100)
		})
		return resultValue
	}
</script>
...
<section class="rounded-md border border-gray-600 p-4">
	...
	<ul>
		{#each $mySeals as mySeal (mySeal.seal.id)}
			<li>
				<SealItem seal={mySeal.seal} count={mySeal.count} />
				<button class="btn" on:click={() => mySeals.remove(mySeal.seal.name)}
					>삭제</button
				>
			</li>
		{/each}
